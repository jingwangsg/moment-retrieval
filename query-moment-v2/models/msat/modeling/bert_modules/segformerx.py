...
class SegFormerXAttention(nn.Module):
    ...
    def get_attn_logits(self, feat_k, mask_k, feat_q, mask_q, proj):
        ...
        attn_logits[~attn_mask] = -1000000.0
...
class SegFormerXEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, num_head, ff_dim, sr_ratio, dropout) -> None:
        ...
        self.ff_txt = MLP(d_model, ff_dim, d_model, activation=F.gelu)
        self.ff_vid = MLP(d_model, ff_dim, d_model, activation=F.gelu)
    ...
    def forward(self, vid_feat, vid_mask, txt_feat, txt_mask, vid_pe=None):
        ...
        for idx, layer in enumerate(self.layers):
            if self.use_patch_merge[idx]:
                ...
                vid_mask = self._interpolate_to_same_size(vid_mask[:, :, None].float(), vid_feat.shape[1]).squeeze(-1).bool()
                if vid_pe:
...
class SegFormerX(nn.Module):
    ...
    def __init__(self,
                 ...
                 pe="learn",
                 pe_kernel_size=3,
                 use_patch_merge=[True, False, True, False]) -> None:
        super().__init__()
        print("using segformerx")
        ...
        # self.type_embed_vid.data.normal_(mean=0.0, std=0.02)
    ...
    def forward(self, txt_feat, txt_mask, word_mask, vid_feat):
        B, Lv, _ = vid_feat.shape
        B, Lt, _ = txt_feat.shape
        txt_mask = txt_mask.bool()

        vid_feat, txt_feat, vid_pe = self._get_embedding(vid_feat, txt_feat)
        ...
        return txt_feat, intermediate_states[-1]


class SegFormerXFPN(nn.Module):

    def __init__(self, backbone, intermediate_hidden_size=[1024, 1024, 1024, 1024], fpn_hidden_size=256) -> None:
        super().__init__()
    ...
    def forward(self, vid_feat, vid_mask, txt_feat, txt_mask):
        intermediate_states, intermediate_masks = self.backbone(vid_feat, vid_mask, txt_feat, txt_mask)
        fpn_states = []

        for idx, adapter in enumerate(self.adapters):
            cur_state = intermediate_states[idx].transpose(1, 2)
