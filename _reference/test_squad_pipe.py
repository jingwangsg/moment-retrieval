...
@functional_datapipe("parse_squad")
class _ParseSQuADQAData(IterDataPipe):
    ...
    def __iter__(self):
        for stream in self.source_datapipe:
                ...
                for layer2 in layer1["paragraphs"]:
                    for layer3 in layer2["qas"]:
                        ...
                        if len(_answers) == 0:
                            _answers = [""]
                            _answer_start = [-1]
                        yield dict(context=_context, question=_question, answers=_answers, answer_start=_answer_start)
...
def SQuAD1():
    ...
    cache_dp = cache_dp.read_from_http().end_caching(mode="wb", same_filepath_fn=True)

    cache_dp = cache_dp.open_files(mode="b")

    # stack custom data pipe on top of JSON reader to orchestrate data samples for Q&A dataset
    return cache_dp.parse_json_files().map(itemgetter(1)).parse_squad().in_memory_cache()


if __name__ == "__main__":
    datapipe = SQuAD1()
    dataloader = DataLoader2(datapipe.batch(16))
    for x in dataloader:
        print(x)
        import ipdb
