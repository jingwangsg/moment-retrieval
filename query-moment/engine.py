...
import torch
from kn_util.basic import add_prefix_dict, save_json, load_json
from kn_util.basic import global_get as G
from kn_util.basic import global_set as GS
from kn_util.data import collection_to_device, collection_apply
...
from kn_util.distributed import get_device
...
@torch.no_grad()
def evaluate(model, loader, evaluater, cfg, compare_topk=False):
    ...
    for idx, batch in enumerate(tqdm(loader, desc="evaluating")):
        ...
        processed_out.extend(post_process(out))

    prediction_fn = osp.join(cfg.paths.work_dir, "./prediction.json")

    if compare_topk:
        topk1 = [_["topk_indices"][0] for _ in processed_out]

        if osp.exists(prediction_fn):
            last_prediction = load_json(prediction_fn)
            topk1_last = [_["topk_indices"][0] for _ in last_prediction]
            n = 0
            hit = 0
            for topk1_item, topk1_last_item in zip(topk1, topk1_last):
                if topk1_item == topk1_last_item:
                    hit += 1
                n += 1
            G("logger").info(f"topk1_consistency: {hit/n:.4f}")

    save_json(processed_out, osp.join(cfg.paths.work_dir, "./prediction.json"))
...
def train_one_epoch(
    ...
    for idx, batch in enumerate(train_loader):
        ...
        batch = collection_to_device(batch, device)
        ...
        if use_amp:
            ...
            loss.backward()
            grad_norm = None if not cfg.train.clip_grad else nn.utils.clip_grad_norm_(
                model.parameters(), max_norm=cfg.train.clip_grad).item()
            optimizer.step()
        ...
        if use_validate:
            st = time.time()
            metric_vals = evaluate(model, val_loader, val_evaluater, cfg, compare_topk=True)
