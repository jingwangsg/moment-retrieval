...
class SegFormerXEncoder(nn.Module):
    ...
    def __init__(self, d_model_in, d_model_lvls, num_head_lvls, sr_ratio_lvls, ff_dim_lvls, dropout) -> None:
        super().__init__()
...
class SegFormerX(nn.Module):
    ...
    def _get_embedding(self, vid_feat, txt_feat):
        ...
        txt_feat = self.txt_ln(txt_feat + self.txt_pe.weight[None, :txt_feat.shape[1]] + self.type_embed_txt)
    ...
    def forward(self, vid_feat, vid_mask, txt_feat, txt_mask):
        ...
        intermediate_states, intermediate_masks = self.encoder(vid_feat, vid_mask, txt_feat, txt_mask)
