...
class SRAttention(nn.Module):
        ...
        if sr_ratio > 1:
            self.sr = nn.Sequential(
                ...
                nn.Conv1d(
                    ...
                    stride=sr_ratio), Rearrange("b d lv -> b lv d"))
    ...
    def forward(self, vid_feat, vid_mask):
        if self.sr_ratio > 1:
            ...
            vid_mask_sr = F.interpolate(
                vid_mask[:, None, :].float(),
                size=vid_feat_sr.shape[1]).bool()
        ...
        attn_mask = repeat(
            attn_mask, "b lq lk -> (b nh) lq lk",
            nh=self.num_head)  # make native module happy
        context, _ = self.mhattn(
            vid_feat,
            vid_feat_sr,
            vid_feat_sr,
            need_weights=False,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, d_model, num_head, ff_dim, sr_ratio, dropout) -> None:
        ...
        self.patch_merge = nn.Sequential(
            Rearrange("b lv d -> b d lv"), nn.Conv1d(d_model, d_model, 3, 2,
                                                     1),
    ...
    def forward(self, vid_feat, vid_mask):
        ...
        vid_mask = F.interpolate(
            vid_mask[:, None, :].float(), size=vid_feat.shape[1]).bool()
        vid_mask = vid_mask[:, 0, :]
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            BasicBlock(
                ...
                ff_dim=ff_dim,
                sr_ratio=sr_ratio,
                dropout=dropout) for sr_ratio in sr_ratio_lvls
        ])
