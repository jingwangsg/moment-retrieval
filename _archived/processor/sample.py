...
class SequenceSampler:
    ...
    def __init__(self, axis=0, max_len=None, stride=None, from_key=None, pooling="none") -> None:
    ...
    def __call__(self, result):
        ...
        if self.pooling == "none":
            slices = [slice(0, seq.shape[_]) if _ != axis else slice(0, tot_len - 1, stride) for _ in range(dim_shape)]
            sampled_seq = seq[tuple(slices)]
            slices[axis] = -1
            last = seq[tuple(slices)]
            result[self.from_key + ".sample"] = np.concatenate([sampled_seq, np.expand_dims(last, axis)], axis=axis)
        # elif self.pooling == "mean":
        #     for idx in range(0, tot_len, stride):
        #     sampled_seq = np.mean(seq, axis=axis)
